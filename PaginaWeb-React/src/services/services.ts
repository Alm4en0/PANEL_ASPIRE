...
export const sendDataLogin = async (pEmail: string, pPws: string) => {
  try {
    const res = await axios.post(`${API_LINK}/api/user/${pEmail}`, {
      Contraseña: pPws,
    });
    return res.data;
  } catch (error: any) {
...
export const reserveCourse = async (
  pNombreDeUsuario: string | undefined,
  pIdCourse: string | undefined
) => {
  try {
    console.log("Se reservo el curso");
    if (pNombreDeUsuario !== undefined && pIdCourse !== undefined) {
      const res = await axios.post(`${API_LINK}/api/reserve`, {
        CursoId: pIdCourse,
        NombreDeUsuario: pNombreDeUsuario,
      });
      return res.data;
    }
    return null;
...
export const saveChangesCourse = async (pCourse: Course) => {
  try {
    ...
    const res = await axios.patch(
      `${API_LINK}/api/courses/${pCourse.Id}`,
      pCourse
    );
    return res.data;
  } catch (err: any) {
...
export const changeStateCourse = async (pId?: string, pState?: string) => {
  try {
    // console.log(pCourse)
    const res = await axios.post(`${API_LINK}/api/changeStateCourse/${pId}`, {
      Estado: pState,
    });
    return res.data;
  } catch (err: any) {
...
export const getNumReservasDeCursoTitulo = async (titulo: string) => {
  try {
    ...
    const res = await axios.post(
      `${API_LINK}/api/data/ReservasDeCursoPorTitulo/${titulo}`
    );
    return res.data;
  } catch (err: any) {
    console.log(err.response);
